...
# --- Treasury Selector Logic ---
def test_treasury_selector_short_term():
    # 6 months (0.5 years) should use 13-Week Bill
    ticker, name = select_treasury_ticker(0.5)
    assert ticker == "^IRX"
    assert "13-Week" in name

def test_treasury_selector_medium_term():
    # 3 years should use 5-Year Note
    ticker, name = select_treasury_ticker(3.0)
    assert ticker == "^FVX"
    assert "5-Year" in name

def test_treasury_selector_long_term():
    # 20 years should use 30-Year Bond
    ticker, name = select_treasury_ticker(20.0)
    assert ticker == "^TYX"
